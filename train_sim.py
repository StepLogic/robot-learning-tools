...
from wrappers import EnvCompatibility, Logger, MobileNetFeatureWrapper, RewardWrapper, save_checkpoint,StackingWrapper,GoalRelObservationWrapper
...
def load_replay_buffer(replay_buffer, path, step):
    """Load replay buffer from checkpoint."""
    try:
        replay_buffer_path = os.path.join(path, f"replay_buffer_step_{step}.pkl")
        if os.path.exists(replay_buffer_path):
            with open(replay_buffer_path, 'rb') as f:
                buffer_data = pickle.load(f)
            
            replay_buffer._storage = buffer_data['data']
...
def main(_):
    ...
    env = GoalRelObservationWrapper(env)
